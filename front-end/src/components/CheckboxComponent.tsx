...
const CheckboxComponent = ({ question, listId, questionId }: Props) => {
  ...
  return (
    <div>
      ...
      {validOptions.map((option, index) => (
        <div key={index}>
          <input type="checkbox" name={option.xpath} value={option.label} />
          <label htmlFor={option.xpath}>
            {option.label}
            <input
              style={{ width: "50px" }}
              type="number"
